...
for iter_idx, maxiter in enumerate(np.linspace(200,2000,10)):
    for reg_idx, reg_lambda in enumerate(np.hstack((np.linspace(0.01,0.1,3,endpoint=False),
        ...
        for train_idx in np.arange(5):
            nn = MLP_1HL(reg_lambda=reg_lambda,epsilon_init=0.16,hidden_layer_size=25,opti_method='TNC',maxiter=int(maxiter),load_theta0=True)
        ...
        result[iter_idx][reg_idx] = train_result.mean(axis=0)
